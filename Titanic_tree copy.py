...
def clean_scale_data(csv, scaler):
    ...
    test_data.loc[df['Age'].isnull()]= df['Age'].mean()
    df.loc[(df['Age'].isnull())&(df['Survived']==0), 'Age']= 30.62
...
test_data, test_data_scaled = clean_scale_data("test (1) - test (2).csv", scaler)

model = LogisticRegression(max_iter= 500, penalty='l2',C=.5)
model.fit(X_train_scaled, y_train)
pred= model.predict(test_data_scaled)

merged=test_data.merge(pd.DataFrame(pred), left_on=None, right_on=None, left_index=True, right_index=True)
merged.head()
output = pd.DataFrame({'PassengerId': merged['PassengerId'], 'Survived':merged[0] })
output.to_csv('lr_team6.csv', index=False)
output.head()
